#! /usr/bin/env python3
"""
script6.py: Este programa pasa los números a texto.
usage as a script: script6.py number
example: ./script6.py 8
"""
import sys
# conversion mappings
...
def num2words(num_string):
    if num_string == '0':
        return'zero'
    if len(num_string) > 2:                         # Verificamos que sea un n° de 2 digitos
        return "Sorry can only handle 1 or 2 digit numbers"
    num_string = '0' + num_string                   # A cualquier n° le agregamos un 0 adelante
    tens, ones = num_string[-2], num_string[-1]     # Asigna cada digito a una variables
    if tens == '0':
